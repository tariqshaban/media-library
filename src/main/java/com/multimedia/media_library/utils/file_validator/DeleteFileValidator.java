...
@Component
public class DeleteFileValidator implements FileValidator<String> {
    @Override
    public List<Violation> validate(String directory, String filename) {
        ...
        if (!Files.exists(path)) {
            violations.add(new Violation(String.format("File named \"%s\" cannot be deleted since it does not exist.", filename)));
        }
